...
exports.describe = function (text) {
  return {
    ...
    //
    _request: function (/* method [uri, data, params, options] */) {
      var self    = this,
      ...
      // and set it to the `json` property of the options.
      //
      // If a options object was passed the `dataType` key will
      // determine how the data is handled. `multipart` expects
      // an array of parts. A part is an object with a `body`
      // property and can optionally contain properties for headers
      // that will be included in that part. `urlencoded` will
      // do a default x-www-form-urlencoded encoding on the data
      // (Currently can't handle nested structures). `raw` doesn't
      // do any encoding of the data. Any other values for `dataType`
      // default to a JSON encoding.
      //
